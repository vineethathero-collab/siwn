...
// Helper to clean and normalize category
function normalizeCategory(str) {
    if (!str) return '';
    let clean = String(str).replace(/\.[a-zA-Z0-9]+$/, '').trim().toLowerCase();
    
    // Match standard categories and their Sinhala/English variations
    if (clean === 'pinkam' || clean === 'pinkama' || clean === 'pin' || clean.includes('පින්') || clean === 'meritorious' || clean === 'merit') {
        return 'pinkam';
    }
    if (clean === 'events' || clean === 'event' || clean === 'dhamma' || clean === 'dharma' || clean.includes('ධම්ම') || clean.includes('දම්') || clean === 'උත්සව') {
        return 'events';
    }
    if (clean === 'other' || clean.includes('වෙනත්') || clean === 'general') {
        return 'other';
    }
    return clean;
}

// Helper to parse Category and Date using delimiter '|', '#', or '-'
function parseCategory(rawItem, fallbackCategory = 'other') {
    let rawStr = rawItem.title || rawItem.name || rawItem.filename || rawItem.description || '';
    let itemDate = rawItem.date || rawItem.createdTime || null;
    let extractedCategory = '';

    // Check delimiter: '|', '#', or ' - '
    const delimiterMatch = rawStr.match(/(\s*\|\s*|\s*#\s*|\s+-\s+)/);

    if (delimiterMatch) {
        const delimiter = delimiterMatch[0];
        const parts = rawStr.split(delimiter).map(p => p.trim()).filter(Boolean);

        for (let i = 0; i < parts.length; i++) {
            const part = parts[i];
            const dateMatch = part.match(/\b(\d{4}-\d{2}-\d{2})\b/);
            if (dateMatch && !itemDate) {
                itemDate = dateMatch[1];
                continue;
            }

            const cleanPart = part.replace(/\.[a-zA-Z0-9]+$/, '').trim();
            const norm = normalizeCategory(cleanPart);

            if (['pinkam', 'events', 'other'].includes(norm)) {
                extractedCategory = norm;
            } else if (!extractedCategory && cleanPart) {
                extractedCategory = norm || cleanPart;
            }
        }
    } else {
        // No explicit delimiter: check if date is present in string
        const dateMatch = rawStr.match(/\b(\d{4}-\d{2}-\d{2})\b/);
        if (dateMatch) {
            if (!itemDate) itemDate = dateMatch[1];
            let rest = rawStr.replace(dateMatch[0], '').replace(/\.[a-zA-Z0-9]+$/, '').replace(/^[\s_\-#|]+|[\s_\-#|]+$/g, '').trim();
            if (rest) {
                const norm = normalizeCategory(rest);
                if (norm) extractedCategory = norm;
            }
        }
    }

    // Check rawItem.category if not extracted from filename
    if (!extractedCategory && rawItem.category && rawItem.category !== 'undefined') {
        extractedCategory = normalizeCategory(rawItem.category);
    }

    // If still not recognized as standard category, check rawItem.category as fallback
    if (!['pinkam', 'events', 'other'].includes(extractedCategory)) {
        if (rawItem.category && rawItem.category !== 'undefined') {
            const normRaw = normalizeCategory(rawItem.category);
            if (['pinkam', 'events', 'other'].includes(normRaw)) {
                extractedCategory = normRaw;
            }
        }
    }

    const category = extractedCategory || fallbackCategory;

    return { category, date: itemDate };
}

// Category Sinhala/English Label Helper
function getCategoryLabel(cat) {
    const isEn = document.documentElement.lang === 'en' || (window.location && window.location.pathname.includes('-en'));
    if (!cat) return isEn ? 'Other' : 'වෙනත්';
    const c = String(cat).trim().toLowerCase();
    if (c === 'events' || c === 'dhamma') return isEn ? 'Dhamma' : 'ධම්ම';
    if (c === 'pinkam' || c === 'meritorious') return isEn ? 'Meritorious' : 'පින් කටයුතු';
    if (c === 'other') return isEn ? 'Other' : 'වෙනත්';
    return cat;
}
...
function readCachedImages() {
    ...
    try {
        ...
        const fresh = rawItems.length > 0 && Date.now() - timestamp < CACHE_TTL_MS;

        const items = rawItems.map(img => {
            const parsedCat = parseCategory(img, 'other');
            return {
                ...img,
    ...
    if (cachedVideoData) {
        try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                allVideosData = parsed.map(v => {
                    const p = parseCategory(v, 'other');
                    return {
                        ...v,
                        category: p.category,
                        date: p.date
...
function loadNextBatch() {
    ...
    nextBatch.forEach((img, index) => {
        ...
        card.className = 'group relative bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer border border-slate-100 opacity-0 flex flex-col justify-between';
        ...
        card.innerHTML = `
                    <div class="aspect-[4/3] w-full overflow-hidden bg-slate-100 relative">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-950/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                            <span class="text-white text-xs font-semibold bg-amber-500/90 px-3 py-1 rounded-full backdrop-blur-sm"> 🔍</span>
                        </div>
                    </div>
                `;
...
function showLoadingState() {
    const container = document.getElementById('photoGallery');
    const isEnglishPage = document.documentElement.lang === 'en' || window.location.pathname.includes('-en');
    const loadingMessage = isEnglishPage ? 'Dhamma photos are loading...' : 'සදහම් ඡායාරූප එක්රැස් වෙමින් පවතී...';
    container.innerHTML = `
